...
struct Student {
    ...
    // onstructor por defecto para inicializar valores limpios
    Student() : student_id(0), semester(0), gpa(0.0f), skill_score(0), projectsHead(nullptr) {}

    //Destructor: libera todos los proyectos del estudiante
    ~Student() {
        Project* current = projectsHead;

        while(current) {
            Project* temp = current;
    ...
    //Buscar un proyecto específico por ID dentro de la lista enlazada
...
//Contar cuántos proyectos tiene registrados este estudiante
int countProjects() {
    int total = 0;
